...
// GROUP MEMBER CACHE
// =====================================================
// Fetch group members only when:
// - there is no cache
// - cache has expired
// Then requested numbers are checked locally.
// 5 minutes = 300000 ms
...
// Prevent multiple refreshes of the same group
...
async function updateSessionAccount(sessionToken, accountNumber) {
    if (!db || !sessionToken || !accountNumber) {
        return;
    }
    try {
        await db.execute(`
...
async function getRecentChatsDirect() {
    ...
    const result = await client.pupPage.evaluate(() => {
        ...
        return models.map(chat => {
            ...
            let name = 'Unknown';
            try {
                name = chat.formattedTitle || chat.name || chat.pushname || chat.id?.user || 'Unknown';
            } catch (_) {}
...
function normalizeLocalPhoneNumber(value) {
    ...
    digits = digits.replace(/^0+/, ''); // Remove leading zero(s)
...
function extractSerializedId(participant) {
    try {
        if (participant?.id?._serialized) return String(participant.id._serialized);
        if (participant?.id?.user && participant?.id?.server) {
            return `${participant.id.user}@${participant.id.server}`;
        }
        if (participant?.id?.user) return String(participant.id.user);
    } catch (_) {}
    return null;
}

function phoneDigitsFromSerializedId(serializedId) {
    if (!serializedId) return null;
    const value = String(serializedId);
    if (!value.endsWith('@c.us')) return null;
    
    const digits = value.slice(0, -5).replace(/\D/g, '');
    return digits || null;
}

async function fetchGroupParticipantsWithPhones(groupId) {
    if (!client.pupPage) {
        throw new Error('WhatsApp browser page is not ready.');
    }
    
    return await client.pupPage.evaluate(async requestedGroupId => {
        const collections = window.require('WAWebCollections');
        const widFactory = window.require('WAWebWidFactory');
        const groupQuery = window.require('WAWebGroupQueryJob');
        
        const groupWid = widFactory.createWid(requestedGroupId);
        let group = collections.Chat.get(groupWid);
        
        if (!group) {
            group = await collections.Chat.find(groupWid);
        }
        if (!group) {
            throw new Error('Target group was not found in WhatsApp Web.');
        }
        
        try {
            await groupQuery.queryAndUpdateGroupMetadataById({ id: requestedGroupId });
        } catch (error) {
            console.warn('Group metadata refresh warning:', error?.message || String(error));
        }
        ...
        for (const participant of serializedParticipants) {
            const id = participant?.id;
            if (!id) continue;
            
            const serialized = id._serialized || (id.user && id.server ? `${id.user}@${id.server}` : null);
            if (!serialized) continue;
            
            participantIds.push(String(serialized));
            
            if (id.server === 'c.us') {
                const digits = String(id.user || '').replace(/\D/g, '');
                if (digits) phoneNumbers.push(digits);
                continue;
            }
            
            if (id.server === 'lid') {
                unresolvedLids.push(String(serialized));
                try {
                    let resolved = null;
                    if (window.WWebJS && typeof window.WWebJS.enforceLidAndPnRetrieval === 'function') {
                        resolved = await window.WWebJS.enforceLidAndPnRetrieval(serialized);
                    }
                    
                    const phoneId = resolved?.phone?._serialized || (
                    ...
                    if (phoneId && phoneId.endsWith('@c.us')) {
                        const digits = phoneId.slice(0, -5).replace(/\D/g, '');
                        if (digits) phoneNumbers.push(digits);
                    }
                } catch (error) {
                    console.warn(`Could not resolve LID ${serialized}:`, error?.message || String(error));
                }
            }
        }
        
        return {
            groupName: String(group.formattedTitle || group.name || 'Unknown Group'),
            participantIds,
            phoneNumbers,
            unresolvedLidCount: unresolvedLids.length,
            participantCount: serializedParticipants.length
        };
    }, groupId);
}

async function fetchGroupMemberCache(groupId) {
    if (!whatsappReady) {
        throw new Error('WhatsApp is not ready. Please wait or relogin.');
    }
    
    const normalizedGroupId = String(groupId || '').trim();
    if (!normalizedGroupId.endsWith('@g.us')) {
        throw new Error('Invalid Group ID. It must end with @g.us.');
    }
    
    console.log('Refreshing group member cache:', normalizedGroupId);
    const data = await fetchGroupParticipantsWithPhones(normalizedGroupId);
    
    const memberNumbers = new Set(data.phoneNumbers);
    const cacheEntry = {
        groupId: normalizedGroupId,
        groupName: data.groupName || 'Unknown Group',
        memberNumbers,
        memberCount: data.participantCount,
        resolvedPhoneCount: memberNumbers.size,
        unresolvedLidCount: data.unresolvedLidCount,
        fetchedAt: Date.now()
    };
    
    groupMemberCache.set(normalizedGroupId, cacheEntry);
    console.log(`GROUP CACHE REFRESHED: ${normalizedGroupId} | participants=${cacheEntry.memberCount} | resolvedPhones=${cacheEntry.resolvedPhoneCount} | unresolvedLids=${cacheEntry.unresolvedLidCount}`);
    
    return cacheEntry;
}

async function getGroupMemberCache(groupId) {
    const normalizedGroupId = String(groupId || '').trim();
    const existing = groupMemberCache.get(normalizedGroupId);
    const now = Date.now();
    
    if (existing && (now - existing.fetchedAt) < GROUP_MEMBER_CACHE_TTL_MS) {
        return { ...existing, fromCache: true };
    }
    
    if (groupMemberRefreshes.has(normalizedGroupId)) {
        const refreshed = await groupMemberRefreshes.get(normalizedGroupId);
        return { ...refreshed, fromCache: false };
    }
    
    const refreshPromise = fetchGroupMemberCache(normalizedGroupId);
    groupMemberRefreshes.set(normalizedGroupId, refreshPromise);
    
    try {
        const refreshed = await refreshPromise;
        return { ...refreshed, fromCache: false };
    } finally {
        groupMemberRefreshes.delete(normalizedGroupId);
    }
}

function isPhoneInCachedGroup(cacheEntry, requestedNumber) {
    if (cacheEntry.memberNumbers.has(requestedNumber)) {
        return true;
    }
    for (const memberPhone of cacheEntry.memberNumbers) {
        if (memberPhone.endsWith(requestedNumber)) {
            return true;
        }
    }
    return false;
...
function clearGroupMemberCache() {
    ...
    console.log('GROUP MEMBER CACHE CLEARED.');
}
...
io.on('connection', async socket => {
    ...
    socket.on('checkGroupMembership', async data => {
        try {
            ...
            const number = normalizeLocalPhoneNumber(data?.number);
            ...
            if (!groupId) {
                socket.emit('groupMembershipError', 'Group ID is required.');
                return;
            }
            
            const cache = await getGroupMemberCache(groupId);
            const isMember = isPhoneInCachedGroup(cache, number);
            
            const cacheAgeSeconds = Math.max(0, Math.floor((Date.now() - cache.fetchedAt) / 1000));
            ...
            socket.emit('groupMembershipResult', {
                ...
                resolvedPhoneCount: cache.resolvedPhoneCount,
                unresolvedLidCount: cache.unresolvedLidCount,
                fromCache: Boolean(cache.fromCache),
                cacheAgeSeconds,
                cacheTtlSeconds: Math.floor(GROUP_MEMBER_CACHE_TTL_MS / 1000)
            });
            
            console.log(`GROUP CHECK: ${number} => ${isMember ? 'MEMBER' : 'NOT MEMBER'} | ${cache.fromCache ? 'CACHE HIT' : 'CACHE REFRESH'}`);
        } catch (error) {
            console.error('checkGroupMembership error:', error);
            socket.emit('groupMembershipError', `Failed to check membership: ${error?.message || String(error)}`);
        }
    });
    ...
    // =================================================
    socket.on('checkGroupMembershipMany', async data => {
        try {
            const groupId = String(data?.groupId || '').trim();
            
            if (!whatsappReady) {
                socket.emit('groupMembershipManyError', 'WhatsApp is not ready. Please wait or relogin.');
                return;
            }
            if (!groupId) {
                socket.emit('groupMembershipManyError', 'Group ID is required.');
                return;
            }
            
            const rawNumbers = Array.isArray(data?.numbers) ? data.numbers : [];
            if (!rawNumbers.length) {
                socket.emit('groupMembershipManyError', 'At least one phone number is required.');
                return;
            }
            if (rawNumbers.length > 500) {
                socket.emit('groupMembershipManyError', 'Maximum 500 numbers per request.');
                return;
            }
            
            const cache = await getGroupMemberCache(groupId);
            
            const results = rawNumbers.map(rawNumber => {
                try {
                    const number = normalizeLocalPhoneNumber(rawNumber);
                    return {
                        number,
                        isMember: isPhoneInCachedGroup(cache, number)
                    };
                } catch (error) {
                    return {
            ...
            socket.emit('groupMembershipManyResult', {
                groupId: cache.groupId,
                groupName: cache.groupName,
                results,
                memberCount: cache.memberCount,
                ...
                unresolvedLidCount: cache.unresolvedLidCount,
                fromCache: Boolean(cache.fromCache),
                cacheAgeSeconds: Math.max(0, Math.floor((Date.now() - cache.fetchedAt) / 1000)),
                cacheTtlSeconds: Math.floor(GROUP_MEMBER_CACHE_TTL_MS / 1000)
            });
        } catch (error) {
            console.error('checkGroupMembershipMany error:', error);
            socket.emit('groupMembershipManyError', `Failed to check memberships: ${error?.message || String(error)}`);
        }
    });
...
client.on('message', async message => {
    try {
        ...
        if (!chatId || chatId.endsWith('@g.us')) return; // Ignore groups
